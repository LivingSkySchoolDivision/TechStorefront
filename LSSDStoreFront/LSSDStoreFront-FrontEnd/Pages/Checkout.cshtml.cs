...
namespace LSSD.StoreFront.FrontEnd.Pages
{
    public class CheckoutModel : PageModel
    {
        ...
        public IActionResult OnPostPlaceOrder()
        {
            ...
            string submittedByFullName = ((System.Security.Claims.ClaimsIdentity)User.Identity).Claims.Where(c => c.Type == "name").FirstOrDefault().Value;
            string submittedByEmail = ((System.Security.Claims.ClaimsIdentity)User.Identity).Claims.Where(c => c.Type == "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress").FirstOrDefault().Value;
            UserThumbprint userThumb = new UserThumbprint(User.Identity.Name);

            // Create the order
            Order newOrder = orders.CreateOrder(shoppingCart.Items, budgetNumber, submittedByFullName, submittedByEmail, userThumb, customerNotes);
